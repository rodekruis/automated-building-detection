...
from abd_model.da.core import to_tensor_instseg
...
class InstSeg(torch.utils.data.Dataset):
    def __init__(self, config, ts, root, cover=None, tiles_weights=None, mode=None, metatiles=False, keep_borders=False):
        ...
        if self.mode in ["train", "eval"]:
            ...
            path = os.path.join(root, "labels_borders")
            self.tiles["labels_borders"] = [(tile, path) for tile, path in
                                    tiles_from_dir(path, cover=self.cover, xyz_path=True)]
            ...
            self.tiles["labels_borders"].sort(key=lambda tile: tile[0])
    ...
    def __getitem__(self, i):
        ...
        if self.mode in ["train", "eval"]:
            ...
            assert tile == self.tiles["labels_borders"][i][0], "Dataset border mask inconsistency"

            mask = tile_label_from_file(self.tiles["labels"][i][1])
            assert mask is not None, "Dataset mask not retrieved"

            mask_border = tile_label_from_file(self.tiles["labels_borders"][i][1])
            assert mask_border is not None, "Dataset border mask not retrieved"
